...
$('.topline .container').html('<span>Racks, gabinetes e acessórios para TI e Telecom</span><a href="#contato">Conte com a Metall Rack <span aria-hidden="true">↗</span></a>');
...
$('.section-heading .eyebrow').text('CATÁLOGO METALL RACK');
...
$('.about-mark').remove();
$('.about .eyebrow').text('SOBRE A METALL RACK');
...
$('footer').html(`<div class="container footer-grid"><div>${brand}<p>Estruturas para tecnologia,<br>telecomunicações e energia.</p></div><div><h3>Nossas linhas</h3>${lines.map(l=>`<a href="#produtos" data-line="${l.id}">${l.name}</a>`).join('')}</div><div><h3>Metall Rack</h3><a href="#sobre">A empresa</a><a href="#guia">Ajuda para escolher</a><a href="#contato">Solicitar orçamento</a></div><div><h3>Vamos conversar?</h3><p>Conte qual é o seu desafio.<br>Vamos encontrar a estrutura certa.</p><a class="footer-contact" href="#contato">Fale sobre seu projeto ↗</a></div></div><div class="container footer-bottom"><span>© 2026 Metall Rack</span><a href="#inicio">Voltar ao topo ↑</a></div>`);
...
$('title').text('Metall Rack | Racks e Gabinetes para TI e Telecom');
