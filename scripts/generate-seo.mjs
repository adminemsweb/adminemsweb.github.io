...
const mainPages=[
  {route:'/',title:'Metal Rack | Racks e Gabinetes para TI e Telecom',description:'Racks, gabinetes, acessórios e soluções para fibra. Conheça a Metal Rack em Sorocaba e solicite orçamento para seu projeto de TI, telecom e energia.',keywords:['racks para TI','racks telecom','gabinetes para equipamentos','Metal Rack']},
  {route:'/produtos/',title:'Produtos | Racks, Gabinetes e Acessórios | Metal Rack',description:'Explore 31 produtos Metal Rack: racks, gabinetes, bandejas, acessórios e fibra. Filtre por linha e solicite orçamento com atendimento em Sorocaba.',keywords:['catálogo de racks','gabinetes','acessórios para rack']},
  {route:'/linhas/',title:'Linhas de Racks, Energia, Acessórios e Fibra | Metal Rack',description:'Conheça as quatro linhas Metal Rack: racks e telecom, gabinetes e energia, acessórios e fibra. Encontre a estrutura para a sua instalação.',keywords:['linhas de racks','gabinetes de energia','acessórios e fibra']},
  {route:'/empresa/',title:'Metal Rack em Sorocaba | Conheça a Empresa',description:'Conheça a Metal Rack em Sorocaba, SP. Portfólio de racks, gabinetes e acessórios para TI, telecom e energia, com entregas em território brasileiro.',keywords:['Metal Rack Sorocaba','racks em Sorocaba','empresa de racks']},
  {route:'/ajuda/',title:'Como Escolher Racks e Gabinetes | Guia Metal Rack',description:'Precisa escolher um rack ou gabinete? Explore nosso guia por aplicação e saiba quais informações enviar para consultar o modelo e solicitar orçamento.',keywords:['como escolher rack','qual rack comprar','escolher gabinete']},
];
const pages=[...mainPages,...products.map(product=>({route:productPath(product,products),title:`${product.title} | ${categoryNames[product.category]} | Metal Rack`,description:description(`${product.title} na linha ${categoryNames[product.category]} da Metal Rack. ${product.description} Consulte configurações e orçamento.`),keywords:[product.title,categoryNames[product.category],`${product.title} orçamento`],product}))];
...
for(const page of pages) {
  ...
  for(const [key,value] of Object.entries({title:page.title,description:page.description,url:absolute(page.route),type:'website',site_name:config.name,locale:'pt_BR',image:absolute(socialImage),'image:alt':page.product?.title || 'Metal Rack: racks, gabinetes, acessórios e fibra'})) add(`og:${key}`,value,true);
