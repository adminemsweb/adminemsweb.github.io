...
A compilação e a integridade dos arquivos locais foram verificadas. A comparação visual em navegador não foi realizada porque não havia navegador conectado disponível.Produ??o: https://metallrack.com.br/ ? VPS 89.116.74.235.

O workflow valida o site e publica na VPS a cada push em main. Pull requests apenas validam. A conex?o SSH usa uma chave exclusiva, restrita ao comando /opt/metallrack/deploy.sh. Secrets: VPS_SSH_KEY e VPS_KNOWN_HOSTS.

O Dockerfile compila e testa as 36 p?ginas. O servi?o metallrack_web usa a rede traefik_public e o resolver de certificados le existente. O DNS A permanece em 89.116.74.235. O c?digo fica em /opt/metallrack/repo; cada imagem recebe o hash do commit. Para voltar ? vers?o anterior: docker service rollback metallrack_web.

O script de entrada /opt/metallrack/deploy.sh ? administrado na VPS; altera??es em deploy/deploy.sh precisam ser instaladas explicitamente nesse caminho. Nenhuma chave privada pertence ao reposit?rio.
